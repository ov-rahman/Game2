...
import {
  addItem, setActive, useActive, recomputeStats, setWeapon, addRelic,
} from '../src/core/items/inventory.js';
...
import {
  WEAPONS, WEAPON_IDS, RELICS, BOSS_RELIC, STARTING_WEAPON, rarityOf,
...
/**
 * Every light must be made of numbers.
 *
 * A single non-finite value anywhere in the light array blacks out the whole
 * scene: the uniform upload silently fails and every fragment comes out unlit.
 * There is no error in the console and nothing to see except a dark screen, so
 * it has cost a full debugging session twice. It gets a check.
 */
function checkLights() {
  ...
  for (let seed = 1; seed <= SEEDS; seed++) {
    for (const def of FLOORS) {
      const d = generateDungeon(new Rng(seed * 977 + def.index), def);
      for (const l of d.lights) {
        checked++;
        for (const k of keys) {
          if (!Number.isFinite(l[k])) {
            if (bad++ === 0) fail(`light on ${def.name} has non-finite ${k}: ${l[k]}`);
          }
        }
      }
    }
  ...
  check(bad === 0, `${bad} lights carry a value that is not a finite number`);
  notes.push(`lights          ${checked} checked, all finite`);
}

/**
 * Weapons and relics: the boss rewards.
 *
 * Weapon stats *replace* the base ones rather than adding to them, so a typo in
 * a stat block does not read as a small imbalance — it reads as a weapon that
 * cannot fire. And every boss owes a relic that exists.
 */
function checkGear() {
  for (const [id, w] of Object.entries(WEAPONS)) {
    check(!!w.name && !!w.desc, `weapon ${id} is missing a name or description`);
    check(!!w.art && w.art.barrel > 0, `weapon ${id} has no viewmodel to build`);
    check(!!rarityOf(w.quality), `weapon ${id} has quality ${w.quality}, which is not a rarity`);
  }
  for (const [id, r] of Object.entries(RELICS)) {
    check(!!r.name && !!r.desc, `relic ${id} is missing a name or description`);
    check(!!r.flags && Object.keys(r.flags).length > 0,
      `relic ${id} sets no flag, so picking it up does nothing`);
  }
  for (const [boss, relicId] of Object.entries(BOSS_RELIC)) {
    check(!!RELICS[relicId], `boss ${boss} drops unknown relic "${relicId}"`);
  }
  check(!!WEAPONS[STARTING_WEAPON], `the starting weapon "${STARTING_WEAPON}" does not exist`);

  // Carry each weapon in turn and make sure the derived stats stay numbers.
  ...
  for (const id of WEAPON_IDS) {
    ...
    for (const k of ['damage', 'fireRate', 'shotSpeed', 'range', 'moveSpeed', 'maxHp']) {
      check(Number.isFinite(st[k]) && st[k] > 0, `weapon ${id} leaves ${k} at ${st[k]}`);
    }
  }
  ...
  check(Number.isFinite(g.player.stats.damage),
    'holding every relic at once produces a non-finite damage stat');

  notes.push(
    `gear            ${WEAPON_IDS.length} weapons, ${Object.keys(RELICS).length} relics, `
    + `${Object.keys(BOSS_RELIC).length}/${FLOORS.length} boss drops`,
  );
}
